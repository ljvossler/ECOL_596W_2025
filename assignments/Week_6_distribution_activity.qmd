...
walker_interval <- read.csv('~/Downloads/walker_count_data.csv')
hist(walker_interval$count)
...
1.  What is the general shape of the data?
    ...
    Looks generally flat or skewed right.
...
3.  Are the data continuous, categorical, binary, or what?
    ...
    Data are discrete and quantitative

### Discuss with the class: What distribution do your data follow?

Are they normal? If not, what are they?

Poisson-Like Distribution due to its discrete values. Likely with more data points, the distribution would become more normal (probably centering between 2-4 walkers per 30-second interval).
...
mean_walkers <- mean(walker_interval$count)
...
rpois(1000, lambda = mean_walkers) %>% hist(main = glue("1000 values from a poisson distribution with mean = {round(mean_walkers, digits=2)}"))
